...
│   ├── scratch-delete-guard.sh — PreToolUse: refuse an rm under a scratchpad root, reply with scratch-rm.sh
...
│   │                             provisioning-check, scratch-delete-check
...
├── bin/                        — commands installed to ~/.claude-workbench/bin/
│   └── scratch-rm.sh           — delete one path beneath a scratchpad root, unprompted
├── scripts/
...
- **allow** — two entries, and deliberately only two: `mcp__plugin_workbench-core_memory__*`, and one installed script at a fixed path, `Bash(bash "$HOME/.claude-workbench/bin/scratch-rm.sh":*)`. Never a shell *pattern*.
...
**Why one script sits there too, and what keeps that from becoming a hole.** `Bash(rm -rf:*)` is in `ask`, so every scratchpad cleanup prompts — the one `rm -rf` an agent runs constantly. That exception cannot be written as a rule: rules are evaluated deny → ask → allow with **first match winning**, specificity does not reorder them, and Bash rules support no negation operator, so `Bash(rm -rf /tmp/:*)` in `allow` is dead text the ask rule beats every time. The pattern that does work is to give the safe operation its own command at a stable path and allow *that*. `bin/scratch-rm.sh` is not named `rm`, so the ask rule is never tested against it, and the real check moves inside the script where it can be logic rather than a glob: one path, resolved physically before the comparison, strictly beneath one of three roots, never a root itself, and a refusal rather than a fallback delete. The roots are the session scratchpad, the login home's `Developer/scratchpad`, and — on Darwin — this account's per-user temporary directory, which is where `mktemp -d` writes and so where an agent's sandbox teardown happens. Off Darwin that third root is not approved at all: `mktemp -d` falls back to `/tmp` there, every account on the machine shares it, and unprompted deletes in another user's work are worse than the prompt this removes. **No root is decided by an environment variable.** A root the caller chooses is not a root: point `$HOME` at a directory holding a `Developer/scratchpad` symlink and every other defence still passes while the wrong tree is deleted, which was reproduced. So the script reads the login home from the password database through bash's `~name` expansion — which no variable and no `PATH` can redirect — and accepts `$HOME` only when it resolves to that same directory. `$TMPDIR` is the same hole under a new name, and gets the same answer: the temporary root comes from `getconf DARWIN_USER_TEMP_DIR`, which answers from the account rather than the environment. When either variable disagrees, that root is dropped and the refusal says why. `hooks/test-scratch-rm.sh` covers the whole check, including that escape, a `..` escape, and a symlink escape that a prefix comparison would wave through. `/workbench-core:setup` installs it to `~/.claude-workbench/bin/`, alongside the dev-team plugin's `approve-commit.sh` and `dispatch-agent.sh`.

**Why the script alone was not enough, and what now carries it into a session.** That command shipped, the rule shipped, and the pair worked — and nobody ran it. Its name appeared in this README, in `rails.json`, in the setup skill, in the script, and in its test, and **not one of those files loads into a session's context.** So every agent reached for `rm -rf` by reflex, hit the `ask` rule, and the user read the prompt the exception exists to remove. Once per cleanup, in every session and every sub-agent.

Two halves close that, and they repeat the argument above one layer up. `hooks/session-warmup.sh` names the command, so an agent knows it *before* it is ever refused one — and it names it *twice, on two channels, because the two reach different readers.* A two-line instruction goes to the hook's SessionStart stdout, and a short section goes into the managed identity block the same hook writes into `~/.claude/CLAUDE.md`. Only the second reaches a *sub-agent*: a freshly spawned one starts with that file in its context and with the hook's stdout absent, which was measured by asking one to introspect before its first tool call. Until that section existed the first channel was the only one, so every sub-agent on the machine started untold and reached for `rm -rf` — and a sub-agent is where a dev-team agent actually runs. Copying the rule into each agent definition instead was rejected: that misses `general-purpose`, `Explore`, `Plan`, and every agent added later, where one managed block reaches all of them at once. `hooks/test-session-warmup.sh` pins each copy against the `rails.json` allow entry that matches it, never against the other copy, so renaming the helper in one place reddens instead of leaving the other channel on a spelling that prompts. **`hooks/scratch-delete-guard.sh` is the enforcing half**, because an instruction is advice: it competes with a reflex, it fades as a session fills, and nothing reports the turns where it lost — the same reason the exception had to become logic rather than a rule. The guard refuses an `rm` whose **every** path sits beneath an approved root and replies with the exact sanctioned spelling, so the agent retries and goes through; that round trip is the accepted cost of the guarantee. It also reaches the deletes no rule can: `rm -r` without `-f`, plain `rm`, and `rmdir` match no entry in `rails.json` at all and fall through to the classifier, which can prompt too.

Three properties keep it from becoming its own hole. **It never deletes anything** — its only outputs are silence and a refusal, because a delete performed from a hook would be a second unprompted delete path outside the reviewed one. **It does not reimplement the root check**: each candidate path is put to the *installed* `scratch-rm.sh` through its `--check` mode, which runs every refusal in that script and stops at the line before the delete, so the two can never disagree and refuse the agent from both sides at once. And **it fails open** like every sibling guard — a missing helper, an unparseable command, a path outside a root, or anything besides a delete in the same command all pass straight through to the prompt they would have hit anyway. The cost of a miss is one prompt, which is the status quo. `hooks/test-scratch-delete-guard.sh` covers it, weighted at the two failures that matter: refusing a delete the helper would *also* refuse (two closed doors and no route), and denying a command `scratch-rm` cannot replace. `Bash(rm -rf:*)` is untouched by all of this and stays exactly where it is.

**The line an allow entry may not cross.** An entry names one plugin's MCP server, or one script at a fixed path under `~/.claude-workbench/bin/`. Neither grants a command *shape*, and that is the whole distinction: `Bash(rm -rf:*)`, or any `*` inside the command part of a Bash entry, grants arbitrary code execution, which is exactly what auto mode suspends and what `autoMode.allow` exists to handle instead. A fixed script path grants the logic in one file this plugin ships, installs, and covers with a test suite CI runs. `hooks/test-permissions.sh` asserts the shape rather than trusting that sentence — every entry is an `mcp__` pattern or a `~/.claude-workbench/bin/*.sh` path spelled exactly, no `Read(` pattern and no `rm` spelling may appear, no wildcard may sit inside a command part, every allowed script must exist in `bin/` **and** have a `hooks/test-<name>.sh` suite that runs it (CI's `hooks/test-*.sh` glob picks that suite up automatically), and no rule may be denied and allowed at once, since deny wins and the allow would be dead text that reads like a grant. *Reviewed* is the one word no assertion holds: a human reads the file before it ships, and nothing here checks that.
