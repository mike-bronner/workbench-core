...
#   * A scratch root itself. Both roots are shared across sessions and hold live
#     state, so the global CLAUDE.md forbids deleting either one.
...
#
# THE APPROVED ROOTS are the two the user's global CLAUDE.md names:
...
scratch_roots() {
  ...
  # reach a glob as a path fragment.
  case "$session" in
    "" | *[!A-Za-z0-9-]*) return 0 ;;
  esac
...
#
# Neither of today's roots contains another, so a single interleaved loop gets
# the same answer and nothing here is reachable through them. A third root is
# all it would take, and whoever adds one should not have to work that out. The
# ordering is a property of these two passes now, not of the current root list.
...
while IFS= read -r ROOT_REAL; do
  ...
  [ "$RESOLVED" != "$ROOT_REAL" ] || refuse \
    ...
    "   Both roots are shared across sessions and hold live state, so deleting one destroys work that is not yours to delete."
done <<< "$ROOTS_REAL"
