...
# server, no repository is touched, and nothing is created or deleted.
#
...
check 2 "the plain form"        "$(bash_json 'git worktree add ../feat')"
check 2 "with a branch flag"    "$(bash_json 'git worktree add -b feat ../feat')"
check 2 "cd then git"           "$(bash_json 'cd /repo && git worktree add x')"
check 2 "git -C"                "$(bash_json 'git -C /repo worktree add x')"
check 2 "two composed -C flags" "$(bash_json 'git -C /repo -C sub worktree add x')"
check 2 "--git-dir before it"   "$(bash_json 'git --git-dir=/repo/.git worktree add x')"
check 2 "-c config before it"   "$(bash_json 'git -c user.name=x worktree add y')"
check 2 "--no-pager before it"  "$(bash_json 'git --no-pager worktree add x')"
check 2 "an absolute git path"  "$(bash_json '/usr/bin/git worktree add x')"
check 2 "sudo"                  "$(bash_json 'sudo git worktree add x')"
check 2 "nice"                  "$(bash_json 'nice git worktree add x')"
check 2 "an env assignment"     "$(bash_json 'GIT_DIR=/repo/.git git worktree add x')"
check 2 "env with a var"        "$(bash_json 'env GIT_PAGER=cat git worktree add x')"
check 2 "after a semicolon"     "$(bash_json 'echo start; git worktree add x')"
check 2 "on the && arm"         "$(bash_json 'true && git worktree add x')"
check 2 "on the || arm"         "$(bash_json 'test -d x || git worktree add x')"
check 2 "a later pipeline stage" "$(bash_json 'echo x | git worktree add y')"
check 2 "bash -c"               "$(bash_json 'bash -c "cd /repo && git worktree add x"')"
check 2 "sh -c"                 "$(bash_json 'sh -c "git worktree add x"')"
check 2 "ssh"                   "$(bash_json 'ssh box "git worktree add x"')"
check 2 "ssh with a port flag"  "$(bash_json 'ssh -p 2222 box "git worktree add x"')"
check 2 "ssh, unquoted command" "$(bash_json 'ssh box git worktree add x')"
check 2 "docker exec"           "$(bash_json 'docker exec -it api git worktree add x')"
check 2 "docker compose exec"   "$(bash_json 'docker compose exec app git worktree add x')"
check 2 "kubectl exec"          "$(bash_json 'kubectl exec pod/api -- git worktree add x')"
check 2 "a second line"         "$(bash_json 'cd /repo
git worktree add x')"
...
check 2 "git worktree remove"    "$(bash_json 'git worktree remove ../feat')"
check 2 "remove --force"         "$(bash_json 'git worktree remove --force ../feat')"
check 2 "git worktree prune"     "$(bash_json 'git worktree prune')"
check 2 "prune with --dry-run"   "$(bash_json 'git worktree prune --dry-run')"
check 2 "remove via -C"          "$(bash_json 'git -C /repo worktree remove x')"
check 2 "prune after a cd"       "$(bash_json 'cd /repo && git worktree prune')"

# ───────────────────────────────────────────────────────────── databases, Bash

echo "blocks a database being created by its own binary:"
check 2 "createdb"               "$(bash_json 'createdb app')"
check 2 "createdb with flags"    "$(bash_json 'createdb -O mike -E UTF8 app')"
check 2 "createuser"             "$(bash_json 'createuser app')"
check 2 "an absolute path"       "$(bash_json '/opt/homebrew/bin/createdb app')"
check 2 "mysqladmin create"      "$(bash_json 'mysqladmin create app')"
...
check 2 "mysqladmin, flags first" "$(bash_json 'mysqladmin -u root -p create app')"
check 2 "createdb after a cd"    "$(bash_json 'cd /repo && createdb app')"
check 2 "createdb through sail"  "$(bash_json 'sail createdb app')"
check 2 "through docker exec"    "$(bash_json 'docker compose exec db createdb app')"
check 2 "through ssh"            "$(bash_json 'ssh box "createdb app"')"
check 2 "through bash -c"        "$(bash_json 'bash -c "createdb app"')"

echo "blocks a database being created from a SQL client payload:"
check 2 "psql -c"                "$(bash_json 'psql -c "CREATE DATABASE app"')"
check 2 "psql --command="        "$(bash_json 'psql --command="CREATE DATABASE app"')"
check 2 "lowercase sql"          "$(bash_json 'psql -c "create database app"')"
check 2 "mysql -e"               "$(bash_json 'mysql -e "CREATE DATABASE app"')"
check 2 "mariadb -e"             "$(bash_json 'mariadb -e "CREATE DATABASE app"')"
check 2 "CREATE SCHEMA"          "$(bash_json 'psql -d app -c "CREATE SCHEMA reporting"')"
check 2 "a quoted identifier"    "$(bash_json 'psql -c "CREATE DATABASE \"app\""')"
check 2 "the second statement"   "$(bash_json 'psql -c "SELECT 1; CREATE DATABASE app"')"
check 2 "echo piped into psql"   "$(bash_json 'echo "CREATE DATABASE app" | psql')"
check 2 "a herestring"           "$(bash_json 'psql <<< "CREATE DATABASE app"')"
check 2 "psql through ssh"       "$(bash_json 'ssh box "psql -c \"CREATE DATABASE app\""')"
check 2 "a heredoc body" "$(bash_json 'psql -d postgres <<SQL
CREATE DATABASE app;
SQL')"
check 2 "a quoted heredoc body" "$(bash_json "psql -d postgres <<'SQL'
CREATE DATABASE app;
SQL")"
check 2 "a double-quoted delimiter" "$(bash_json 'psql -d postgres <<"SQL"
CREATE DATABASE app;
SQL')"
...
check 2 "a dash-stripped heredoc" "$(bash_json 'psql -d postgres <<-SQL
	CREATE DATABASE app;
	SQL')"
check 2 "a dash-quoted heredoc" "$(bash_json "psql -d postgres <<-'SQL'
	CREATE DATABASE app;
	SQL")"
check 2 "a spaced dash heredoc" "$(bash_json 'psql -d postgres <<- SQL
	CREATE DATABASE app;
	SQL')"
check 2 "a spaced delimiter" "$(bash_json 'psql -d postgres << SQL
CREATE DATABASE app;
SQL')"
# The allow side of the same shape, so the fix has not simply moved the failure.
check 0 "a dash heredoc that only reads" "$(bash_json 'psql -d postgres <<-SQL
...
check 0 "a setup file beside a read" "$(bash_json 'cat > setup.sql <<EOF
CREATE DATABASE app;
EOF
psql -c "SELECT 1"')"
check 0 "a heredoc with no client at all" "$(bash_json 'cat > setup.sql <<EOF
CREATE DATABASE app;
EOF')"
...
check 2 "EnterWorktree, no args"  "$(tool_json EnterWorktree '{}')"
check 2 "EnterWorktree by name"   "$(tool_json EnterWorktree '{"name":"feat"}')"
check 2 "EnterWorktree by path"   "$(tool_json EnterWorktree '{"path":".claude/worktrees/x"}')"
check 2 "ExitWorktree remove"     "$(tool_json ExitWorktree '{"action":"remove"}')"
check 2 "remove, discarding"      "$(tool_json ExitWorktree '{"action":"remove","discard_changes":true}')"

echo "blocks an Agent dispatch that makes the harness provision a worktree:"
check 2 "isolation worktree"      "$(tool_json Agent '{"isolation":"worktree","prompt":"x"}')"
check 2 "with a subagent_type"    "$(tool_json Agent '{"subagent_type":"Explore","isolation":"worktree","prompt":"x"}')"
...
check 0 "ExitWorktree keep"       "$(tool_json ExitWorktree '{"action":"keep"}')"
check 0 "ExitWorktree, no action" "$(tool_json ExitWorktree '{}')"
check 0 "an ordinary Agent"       "$(tool_json Agent '{"prompt":"x","subagent_type":"Explore"}')"
check 0 "Agent with a cwd"        "$(tool_json Agent '{"prompt":"x","cwd":"/repo"}')"
...
# startup alone.
check 0 "Agent isolation remote"  "$(tool_json Agent '{"prompt":"x","isolation":"remote"}')"
check 0 "Agent isolation none"    "$(tool_json Agent '{"prompt":"x","isolation":"none"}')"

# ───────────────────────────────────────────── THE PRIORITY: read-only survives

echo "allows every read-only way of inspecting a worktree:"
check 0 "git worktree list"       "$(bash_json 'git worktree list')"
check 0 "list --porcelain"        "$(bash_json 'git worktree list --porcelain')"
check 0 "list via -C"             "$(bash_json 'git -C /repo worktree list')"
check 0 "list after a cd"         "$(bash_json 'cd /repo && git worktree list')"
check 0 "bare git worktree"       "$(bash_json 'git worktree')"
check 0 "git worktree -h"         "$(bash_json 'git worktree -h')"
check 0 "git worktree lock"       "$(bash_json 'git worktree lock ../feat')"
check 0 "git worktree unlock"     "$(bash_json 'git worktree unlock ../feat')"
check 0 "git worktree repair"     "$(bash_json 'git worktree repair')"
check 0 "git worktree move"       "$(bash_json 'git worktree move ../a ../b')"
...
for VERB in status log show diff add commit push pull branch checkout switch; do
  check 0 "git $VERB" "$(bash_json "git $VERB")"
done
check 0 "git commit with a msg"   "$(bash_json 'git commit -m "feat: add worktree docs"')"
check 0 "git log --grep"          "$(bash_json 'git log --grep="worktree add"')"
check 0 "git branch -d"           "$(bash_json 'git branch -d feature/worktree')"

echo "allows every read-only way of inspecting a database:"
check 0 "psql -c SELECT"          "$(bash_json 'psql -c "SELECT * FROM users"')"
check 0 "psql \\l"                "$(bash_json 'psql -c "\l"')"
check 0 "psql, a database name"   "$(bash_json 'psql app')"
check 0 "mysql -e SELECT"         "$(bash_json 'mysql -e "SELECT 1"')"
check 0 "mysql SHOW DATABASES"    "$(bash_json 'mysql -e "SHOW DATABASES"')"
check 0 "mysqladmin status"       "$(bash_json 'mysqladmin status')"
check 0 "mysqladmin ping"         "$(bash_json 'mysqladmin ping')"
check 0 "mysqladmin variables"    "$(bash_json 'mysqladmin -u root variables')"
check 0 "pg_dump"                 "$(bash_json 'pg_dump app > /tmp/app.sql')"
# A literal is data, not a verb. Without literal stripping this SELECT blocks.
check 0 "a SELECT over the words" \
  "$(bash_json "psql -c \"SELECT * FROM audit WHERE msg = 'create database app'\"")"
check 0 "CREATE TABLE"            "$(bash_json 'psql -c "CREATE TABLE users (id int)"')"
check 0 "CREATE INDEX"            "$(bash_json 'psql -c "CREATE INDEX ON users (id)"')"
check 0 "CREATE EXTENSION"        "$(bash_json 'psql -c "CREATE EXTENSION postgis"')"
...
check 0 "touch the file"          "$(bash_json 'touch database/database.sqlite')"
check 0 "sqlite3 CREATE TABLE"    "$(bash_json 'sqlite3 database/database.sqlite "CREATE TABLE users (id int)"')"
check 0 "sqlite3 with -cmd"       "$(bash_json 'sqlite3 app.sqlite ".tables"')"
...
check 0 "sqlite3 fed the matching SQL" \
  "$(bash_json 'echo "CREATE DATABASE other" | sqlite3 app.sqlite')"
check 0 "php artisan migrate"     "$(bash_json 'php artisan migrate')"
check 0 "migrate --seed"          "$(bash_json 'php artisan migrate --seed')"
check 0 "the test suite"          "$(bash_json 'php artisan test')"
check 0 "pest"                    "$(bash_json './vendor/bin/pest')"
...
check 0 "docker compose up"       "$(bash_json 'docker compose up -d')"
check 0 "docker-compose up"       "$(bash_json 'docker-compose up -d')"
check 0 "docker compose create"   "$(bash_json 'docker compose create')"
check 0 "docker run postgres"     "$(bash_json 'docker run -d --name pg postgres:16')"
check 0 "docker volume create"    "$(bash_json 'docker volume create pgdata')"
check 0 "docker network create"   "$(bash_json 'docker network create app-net')"
check 0 "podman compose up"       "$(bash_json 'podman-compose up -d')"
check 0 "sail up"                 "$(bash_json 'sail up -d')"
...
for VERB in db:wipe migrate:fresh migrate:reset migrate:refresh; do
  check 0 "php artisan $VERB" "$(bash_json "php artisan $VERB")"
done
check 0 "dropdb"                  "$(bash_json 'dropdb app')"
check 0 "DROP DATABASE"           "$(bash_json 'psql -c "DROP DATABASE app"')"
check 0 "docker compose down -v"  "$(bash_json 'docker compose down -v')"
...
check 0 "grep for worktree add"   "$(bash_json 'grep -rn "git worktree add" hooks/')"
check 0 "grep for createdb"       "$(bash_json 'grep -rn createdb .')"
check 0 "rg for CREATE DATABASE"  "$(bash_json 'rg "CREATE DATABASE" --glob "*.sql"')"
check 0 "rg for worktree"         "$(bash_json 'rg -n "worktree" hooks/')"
check 0 "echo about createdb"     "$(bash_json 'echo "never run createdb yourself"')"
check 0 "echo about worktrees"    "$(bash_json 'echo "CREATE DATABASE is blocked"')"
check 0 "cat a file so named"     "$(bash_json 'cat docs/create-database.md')"
check 0 "ls the worktree dir"     "$(bash_json 'ls -la .claude/worktrees')"
check 0 "a migration filename"    "$(bash_json 'cat database/migrations/2026_09_17_create_users_table.php')"
check 0 "git log -S"              "$(bash_json 'git log -S createdb --oneline')"
check 0 "an unrelated command"    "$(bash_json 'npm run build')"
check 0 "a create in another tool" "$(bash_json 'gh issue create --title "worktree guard"')"
check 0 "mkdir"                   "$(bash_json 'mkdir -p /tmp/worktree-notes')"
...
check 0 "a Bash call with no input" "$(jq -nc '{tool_name: "Bash"}')"
...
check 0 "an unbalanced quote"      "$(bash_json 'git worktree add "unclosed')"

# ───────────────────────────────────────────────────────────── the messages

echo "explains the block on stderr, per surface:"
OUT=$(bash_json 'git worktree add ../feat' | bash "$GUARD" 2>&1)
...
OUT=$(bash_json 'createdb app' | bash "$GUARD" 2>&1)
assert_contains "names createdb"         "$OUT" "\`createdb\`"
assert_contains "says nothing cleans up" "$OUT" "nothing will clean up"

OUT=$(bash_json 'git worktree remove ../feat' | bash "$GUARD" 2>&1)
assert_contains "names the deletion"     "$OUT" "git worktree remove"
assert_contains "says they were by hand" "$OUT" "set up by hand"

OUT=$(tool_json EnterWorktree '{"name":"feat"}' | bash "$GUARD" 2>&1)
assert_contains "names EnterWorktree"    "$OUT" "EnterWorktree"
assert_contains "points at the cwd"      "$OUT" "Work in the directory you were given"

OUT=$(tool_json ExitWorktree '{"action":"remove"}' | bash "$GUARD" 2>&1)
assert_contains "names the remove action" "$OUT" "action \"remove\""
assert_contains "offers the keep action"  "$OUT" "action \"keep\""

OUT=$(tool_json Agent '{"isolation":"worktree","prompt":"x"}' | bash "$GUARD" 2>&1)
...
for DIR in / /tmp "$HOME"; do
  printf '%s' "$BLOCKED" | (cd "$DIR" && bash "$GUARD") >/dev/null 2>&1
  if [ "$?" = "2" ]; then
...
printf '%s' "$BLOCKED" | (cd "$HOOKS_DIR" && bash ./provisioning-guard.sh) >/dev/null 2>&1
if [ "$?" = "2" ]; then
