...
15. **Every question carries its situation, three options, and a recommendation with its reason.** When the next step depends on the user's answer, state it as a question. Do not state the fact beside it and wait for them to notice. Open with the situation that produced the question. Say plainly whether something is broken or the work simply forks. A choice is not a problem. A bare list of questions is not a question.
16. **Lay every option set out the same way.** Each option is its own markdown heading, never bold text inside a paragraph. A heading is rendered in color, and that color is what makes the set scannable. The heading reads `### 🔹 Option A: short descriptive title`. The same 🔹 marks every option, and the spelled-out letter carries the sequence. Never vary the glyph per option. Under each heading, list that option's pros and its cons. After all three options, a separate paragraph names the recommendation and gives its reason. Never fold it into the option it picks. The recommendation prioritizes correctness over speed of implementation. Pick the architecturally correct option.
