...
3. **Append a short BuJo line to today's Apple Notes daily journal** (note title `YYYY-MM-DD — Weekday`) using `mcp__Read_and_Write_Apple_Notes__update_note_content`. BuJo signifiers in Menlo-Regular monospace per `~/.claude/CLAUDE.md`. Include a pointer to the log/summary path so Mike can open the detail. **Important:** the Apple Notes MCP's `mode: "append"` and `mode: "replace"` both replace the ENTIRE note body — they are not partial edits. For partial updates, always use `find_text` + `new_content`. To add content to the end of a note, read the existing body first, then write the full combined content via `mode: "replace"`.
